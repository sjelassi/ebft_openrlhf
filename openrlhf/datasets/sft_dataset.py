...
class SFTDataset(Dataset):
    ...
    def process_data(self, data):
        ...
        if self.multiturn:
            # (Multiturn logic remains unchanged for brevity, assume standard SFT logic here)
            pass

        # --- MODIFICATION START ---
        # Original code forced response to be empty if pretrain_mode was True.
        # We now allow response extraction even in pretrain_mode.
        prompt, response = preprocess_data(
            data,
            None if self.pretrain_mode else self.input_template,
            self.input_key,
            self.output_key,
            # We likely still want to disable chat templates for pretrain mode
            # unless your pretrain data is specifically chat formatted.
        ...
        return {
            ...
            "response_ranges": None, # Assuming single turn for simple pretrain
        }
    ...
    def __getitem__(self, idx):
        ...
        if not self.pretrain_mode:
            ...
            # Note: You might want to strip the separator space " " if your data 
            # is raw tokens that shouldn't have spaces inserted.
            if response:
                text = (prompt + " " + response)
            else:
                text = prompt
        # --- MODIFICATION END ---
        # print(f"SAMU CHECK: {[text]}", flush=True)
        # wfewef
