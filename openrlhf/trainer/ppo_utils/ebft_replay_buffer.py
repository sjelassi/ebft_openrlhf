...
from openrlhf.trainer.ppo_utils.ebft_experience_maker import Experience
...
class BufferItem:
    ...
    # Sequence tensors
    prompts: torch.Tensor  # X: Original full prompt
    full_sequences: torch.Tensor  #  Full sequence (full prompt + generated)
    doc_ids: torch.Tensor  # B, S
    qa_masks: torch.Tensor  # B, S
...
def split_experience_batch(experience: Experience) -> List[BufferItem]:
    ...
    batch_size = len(experience.prompts)
    ...
    for i in range(batch_size):
        ...
        for k, v in experience.info.items():
            if isinstance(v, (torch.Tensor, list)):
                ...
                # Scalar values are shared across all items in the batch
                item["info"][k] = v
            else:
...
def make_experience_batch(items: List[BufferItem], packing_samples=False) -> Experience:
    ...
    kwargs = {
        key: (
            torch.stack([getattr(item, key) for item in items], dim=0)
...
class EBFTNaiveReplayBuffer(ABC):
